...
void SanPham::themSanPham(){
    ...
    cin >> maSP;
    cin.ignore();       // xoa ki tu newline(\n) trong bo dem sau khi nhap ma san pham , de dam bao lenh getline tiep theo ko bi loi
    ...
    cout << "Nhap so luong: ";
    cin >> soLuong;  
    cout << "Them san pham thanh cong!" << endl;  // sau khi nhap xong thong tin thi in ra dong nay
    SanPham* sp = new SanPham(maSP, tenSP, HSD, giaBan, soLuong);  // tao ra 1 doi tuong san pham co cac thong tin vua nhap va luu tru vao con tro "sp"
    listsp.push_back(*sp);                       // them doi tuong san pham vao ds "listsp" 
                                                 // Lưu ý rằng *sp là đối tượng SanPham mà con trỏ sp trỏ tới.
}
...
void SanPham::xoaSanPham(){
    ...
    cout << "Khong tim thay san pham!" << endl;    // in ra dong nay neu ko nhap ma san pham khong co trong danh sach
}
...
void SanPham::suaSanPham(){
    ...
    for (int i = 0; i < listsp.size(); i++){   // duyet qua tat ca cac san pham trong ds "listsp"
        if (listsp[i].maSP == maSP){            // kiem tra xem ma san pham hien tai co trung voi ma san pham ma nguoi dung da nhap hay khong
            ...
            cout << "Nhap HSD: ";
            getline(cin, listsp[i].HSD);         // cho phep nguoi dung nhap lai HSD
            cout << "Nhap gia ban: ";
            cin >> listsp[i].giaBan;             // cho phep nguoi dung nhap lai gia ban
            cout << "Nhap so luong: ";
            cin >> listsp[i].soLuong;             // cho phep nguoi dung nhap lai so luong
            cout << "Sua san pham thanh cong!" << endl;  // in ra dong nay sau khi sua thanh cong
            return;                           // ket thuc ham khong tiep tuc duyet danh sach nua
        }
    }
    cout << "Khong tim thay san pham!" << endl;   // in ra dong nay neu ko tim thay ma san pham trong danh sach
}
...
void SanPham::hienThi(const int& i){
        ...
        cout << "|   " << listsp[i].HSD ;
        for(int  j=0; j<12-listsp[i].HSD.length(); j++){           /*hien thi han su dung*/
            cout << " ";
        }
...
void SanPham::luuSanPham(){
    ...
    cout << "Luu san pham thanh cong!" << endl;   // in ra dong nay sau khi luu thanh cong vao file
...
void SanPham :: docSanPham(){
    ...
    cout << "Doc san pham thanh cong!" << endl;  // in ra dong nay sau khi doc thanh cong
